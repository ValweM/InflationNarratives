...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Graphical model of base \textsf{keyATM}}
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption{Graphical model of dynamic \textsf{keyATM}}
...
\begin{figure}[H]
	...
	\caption{Modelfit}
	\label{fig:modelfit}
	\floatfoot{Note: The figure visualizes the model fitting. We observe an increase trend for the log-likelihood and an decrease trend for the perplexity, which is an indicator that the model is working as expected \citep{Eshima.2023}.}
\end{figure}
...
%Estimated Alpha
 \begin{figure}[H]
	\begin{center}
		...
		\label{fig:alpha}
		\floatfoot{Note: The figure visualizes $\alpha$, the prior for the document-topic distribution. We let the dynamic keyATM vary $\alpha$ across 4 latent states.}
	\end{center}
\end{figure}
...
\begin{sidewaysfigure}[H]
	...
	\floatfoot{The figure shows the development of the smoothed proportions for both considered corpora. To calculate the relative proportions only the topics with pre-specified keywords were considered.}
\end{sidewaysfigure}
